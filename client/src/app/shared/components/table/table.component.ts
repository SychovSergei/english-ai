...
export class TableComponent<T extends Identifiable> implements OnInit {
  ...
  columnsAdditional: string[] = ['originalText',
    'language',
    'translateValues',
    'voice',];
  ...
  @Input() expandTemplate!: TemplateRef<any>;// = new MatTableDataSource<T>([]);
  ...
  @Input()
  set ceilTemplates(data: { [key: string]: TemplateRef<any> }) {
  ...
  columnsToDisplayWithExpand:  WritableSignal<string[]> = signal(['select',...this.columns(), 'expand','actions']);
  @Input()
  set columnsToDisplayWithExpandInput(val: string[]) {
    this.columnsToDisplayWithExpand.set(['select', ...val, 'expand','actions']);
