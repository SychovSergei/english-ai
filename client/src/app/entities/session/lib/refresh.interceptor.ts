...
export const refreshInterceptor: HttpInterceptorFn = (
  ...
  const loggerService = inject(LoggerService).createLogger('refreshInterceptor');

  // Передаём запрос дальше по цепочке интерсепторов
  return next(req).pipe(
    catchError((error: HttpErrorResponse) => {
      // Проверяем, что это 401 и это НЕ запрос на сам refresh
      loggerService.log('error.status =====', error.status);
      loggerService.log('req.url =====', req.url);
      if (error.status === 401 && !req.url.includes('auth/refresh')) {
        //&& !req.url.includes('auth/login') && !req.url.includes('auth/refresh')
        loggerService.warn('Refresh token process ./././');
        return authService.refreshTokens().pipe(
          switchMap((newToken) => {
            loggerService.log('Refresh NEW token', newToken);
