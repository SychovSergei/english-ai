import {
  ...
  userSchema,
  UserSettings,
} from 'app-old/core/domain/entities';
import {
  IBcryptService,
  ITokenService,
  IUserService,
...
} from 'app-old/core/interfaces';
import { IAuthService, IUserSettingsService } from 'app-old/core/repositories';
...
import { ErrorBody } from '@core/domain/errors';
...
export class AuthService implements IAuthService {
  constructor(
    ...
    @inject(TYPES.ConfigService) private configService: ConfigServicePort,
  ...
  public async refresh(refreshToken: string | undefined, meta: ClientMeta): Promise<Tokens> {
    ...
    if (!settingsId) {
      throw UserError.BadRequest('user', 'user msg', { path: ['settingsId'], message: 'Missing settingsId' });
    }
