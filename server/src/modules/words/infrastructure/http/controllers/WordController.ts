...
@injectable()
export class WordController /* implements WordControllerPort */ {
  constructor(
    @inject(WORDS_TYPES.WordUseCases) private wordUseCases: WordUseCases,
  ...
  getUserWords = async (req: Request, res: Response): Promise<void> => {
    console.log('\x1b[1m\x1b[35m*-*-*-*-*-*- getUserWords -*-*-*-*- START -*-*-*-*-*-*-*-*-*-*-*-\x1b[0m');
    console.log(`\x1b[1m\x1b[4m\x1b[35m WordController\x1b[0m ->` + `\x1b[1m\x1b[4m\x1b[31m getUserWords \x1b[0m`);

    const actor = this.identityProvider.getCurrentActor();
    if (!actor) throw AuthError.Unauthorized();

    const words = await this.wordUseCases.getActorWordsUC.execute();
    // const word = await this.getUserWordsUseCase.execute();

    console.log('*-*-*-*-*-*- getUserWords -*-*-*-*- END -*-*-*-*-*-*-*-*-*-*-*-');

    const response = words.map((w) => WordMapper.toResponseDTO(w, actor));
