...
export const authMiddleware = async (req: CustomRequest, res: Response, next: NextFunction) => {
  ...
  const tokenRepository = container.get<ITokenRepositoryService>(DiTypes.TokenRepositoryService);
  ...
  try {
    ...
    //   const user = await user-model.findOne({ email: decodedRefresh.email }).lean();
    //   if (!user) {
